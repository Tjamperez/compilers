...
LITINT: TK_LIT_INT
    {
        // Create a new AST node for the integer literal
        $$ = ast_new($1);
        $$->node_type = NODE_TYPE_INT;

        // Generate a temporary variable for storing the value
        char* temp = generate_temp();
        $$->temp = temp;

        // Create the assembly instruction for loading the integer literal
        char* op1 = $$->valor_lexico->token_value; // Assuming this is a string representation of the number
        char assembly_code[OPCODE_SIZE_OF_BUFFER];

        // Generate the assembly instruction (e.g., "movl $value, %eax")
        snprintf(assembly_code, sizeof(assembly_code), "movl $%s, %s", op1, temp);

        // Append the assembly code to the AST node
        $$->code = append_assembly_code($$->code, strdup(assembly_code));

        // Optional debug message
        // printf("Added TK_LIT_INT to assembly: %s\n", assembly_code);
    }
;
